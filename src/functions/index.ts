...
export type GetLowPriceData = (client: Client, days: Array<string>, hotels: HotelsData) => string

export const getLowPrice: GetLowPriceData = (client, days = [], hotels = []) => {
    ...
    hotels.map((hotel: HotelData) => {
        ...
        if(currPrice < lowPrice || currPrice === lowPrice && hotel.rate > hotelResult.rate) {
            hotelResult = hotel
            lowPrice = currPrice
        }
    })
